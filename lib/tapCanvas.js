...
async function tapCanvas ({ host, token, account }, state, { streams }) {
  ...
  if (selectedPaths.get('users') !== false) {
    ...
    for await (const user of users) {
      if (selectedPaths.get('users/properties/logins')) {
  ...
  if (selectedPaths.get('courses') !== false) {
    ...
    for (const field of COURSES_INCLUDES) {
      if (selectedPaths.get(`courses/properties/${field}`)) {
  ...
  if (selectedPaths.get('sections') !== false && activeCourseIds.size > 0) {
    ...
    for (const field of SECTION_INCLUDES) {
      if (selectedPaths.get(`sections/properties/${field}`)) {
  ...
  // collect enrollments
  if (selectedPaths.get('enrollments') !== false && sectionIds.size > 0) {
    ...
    for (const field of ENROLLMENT_INCLUDES) {
      if (selectedPaths.get(`enrollments/properties/${field}`)) {
    ...
    for (const sectionId of sectionIds) {
      ...
      for await (const enrollment of enrollments) {
        delete enrollment.last_activity_at
        delete enrollment.total_activity_time
